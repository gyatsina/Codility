...
public class Peaks {
    public int solution(int[] A) {
        ...
        sum[n - 1] = sum[n - 2];
        for (int m = 1; m <= n; m++) {
            if (n % m == 0 && test(sum, m)) return n / m;
        }
        return 0;
    }

    private boolean test(int[] sum, int m) {
        if (sum[m - 1] <= 0) return false;
        for (int i = 2; i <= sum.length / m; i++) {
            if (sum[i * m - 1] - sum[i * m - m - 1] <= 0) return false;
        }
        return true;
    }

    public static void main(String[] args) {
        int ans = new Peaks().solution(new int[]{1, 2, 3, 4, 3, 4, 1, 2, 3, 4, 6, 2});
